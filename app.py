...
with st.sidebar.expander("Regulatory & Competition Controls"):
    patent_revenue_drop_pct = st.slider("Patent Revenue Drop (%)", 0, 50, 20, 5)
    generic_margin_erosion_pct = st.slider("Generic Margin Erosion (%)", 0, 30, 10, 2)
...
price_cap_effect = reg_case["Revenue_scn"] - base_case["Revenue_scn"]
...
wf_fig2.update_layout(
    title="Regulatory Shock: Step-by-Step Profitability Erosion (Dynamic)",
    yaxis_title="$B",
    height=420
)
st.plotly_chart(wf_fig2, use_container_width=True)
st.caption("Adjust the sliders for Price Cap and Compliance Cost to see live profitability impact.")

st.markdown("---")

# ============================
# Objective 3 — IP Expiry & Competition Impact (Dynamic Waterfall)
# ============================
...
# Apply dynamic patent expiry + competition pressure
cliff_mod = df0.copy()
cliff_mod["Revenue"] = cliff_mod["Revenue"] * (1 - patent_revenue_drop_pct/100)
cliff_mod["COGS"] = cliff_mod["COGS"] * (1 + generic_margin_erosion_pct/100)
cliff_mod["Opex_exR&D"] = cliff_mod["Opex_exR&D"] * (1 + generic_margin_erosion_pct/100)

cliff_case = run_scenario(cliff_mod,
                          rd_bps=rd_bps, rd_cash_now_pct=rd_cash_now_pct,
                          new_debt=new_debt, new_equity=new_equity, tax_rate=tax_rate,
                          dso_delta=dso_delta, dpo_delta=dpo_delta, dio_delta=dio_delta,
                          ocf_adj=ocf_adj, preset="Base")

revenue_loss = cliff_case["Revenue_scn"] - base_case["Revenue_scn"]
margin_erosion = -( (cliff_case["COGS"] + cliff_case["Opex_exR&D"]) -
                    (base_case["COGS"] + base_case["Opex_exR&D"]) )
ebit_cliff = cliff_case["EBIT"]

wf_labels = [
    "Revenue (Base)",
    f"Patent Expiry (-{patent_revenue_drop_pct}%)",
    f"Generic Margin Erosion (-{generic_margin_erosion_pct}%)",
    "EBIT (After Cliff)"
]
wf_values = [base_case["Revenue_scn"], revenue_loss, margin_erosion, ebit_cliff]
wf_measures = ["absolute", "relative", "relative", "total"]
...
wf_fig3.update_layout(
    title="Patent Cliff: Step-by-Step Erosion of Profitability (Dynamic)",
    yaxis_title="$B",
    height=420
)
st.plotly_chart(wf_fig3, use_container_width=True)
st.caption(
    f"Adjust the sliders for Patent Revenue Drop (%) and Generic Margin Erosion (%) to see the live impact on profitability."
)
